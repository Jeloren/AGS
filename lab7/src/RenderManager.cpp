...
{
	// Сортируем объекты по материалу, затем по текстуре для оптимизации
	...
	if (shaders.size() > 0) {
		...
		for (const GraphicObject& obj : graphicObjects) {
			// Проверяем смену материала
			if (obj.getMaterialId() != currentMaterialId) {
				currentMaterialId = obj.getMaterialId();
				materialSwitches++;
			}

			// Проверяем смену текстуры
			if (obj.getTextureId() != currentTextureId) {
				currentTextureId = obj.getTextureId();
				textureSwitches++;
			}
...
int RenderManager::getMaterialSwitches() const
{
	return materialSwitches;
}

int RenderManager::getTextureSwitches() const
{
	return textureSwitches;
}

int RenderManager::getDrawCalls() const
{
	return drawCalls;
}
