...
GraphicObject::GraphicObject() :
	...
	textureId(0),
	materialId(-1),
	objectType(ObjectType::SMALL_PROP),
...
void GraphicObject::setObjectType(ObjectType type) {
	this->objectType = type;
}

void GraphicObject::setDimensions(const vec3& dims) {
	this->dimensions = dims;
}

ObjectType GraphicObject::getObjectType() const {
	return objectType;
}

vec3 GraphicObject::getDimensions() const {
	return dimensions;
}
