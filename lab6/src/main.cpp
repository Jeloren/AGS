...
void initGraphicObjects()
{
    ...
    int materialId = rm.loadMaterial("wood");
    ...
    materialId = rm.loadMaterial("plastic");
    graphicObject.setMeshId(meshId);
    graphicObject.setTextureId(textureId);
    graphicObject.setMaterialId(materialId);
    graphicObject.setColor(vec4(1.0f, 1.0f, 1.0f, 1.0f));
    ...
    materialId = rm.loadMaterial("metal");
    graphicObject.setMeshId(meshId);
    graphicObject.setTextureId(textureId);
    graphicObject.setMaterialId(materialId);
    graphicObject.setColor(vec4(1.0f, 1.0f, 1.0f, 1.0f));
    ...
    graphicObject.setMaterialId(materialId);
    graphicObject.setColor(vec4(1.0f, 1.0f, 1.0f, 1.0f)); 
    graphicObject.setPosition(vec3(3.0f, -1.0f, 5.0f)); 
    graphicObject.setAngle(25.0f);
...
int main(int argc, char** argv)
{
	...
	// Устанавливаем источник света (белый) - ЯРКИЙ
	RenderManager::instance().setLight(
		vec3(10.0f, 12.0f, 8.0f),  // position
		vec3(0.5f, 0.5f, 0.5f),    // ambient - сильный амбиент
		vec3(1.5f, 1.5f, 1.5f),    // diffuse - очень яркий
		vec3(1.5f, 1.5f, 1.5f)     // specular - яркий спекуляр
	);

	// Устанавливаем второй источник света (красный) - ЯРКИЙ КРАСНЫЙ
	RenderManager::instance().setLight2(
		vec3(-6.0f, 4.0f, 8.0f),   // position - ближе к центру, под другим углом
		vec3(0.3f, 0.0f, 0.0f),    // ambient - сильный красный амбиент
		vec3(1.5f, 0.1f, 0.1f),    // diffuse - ярко-красный (очень красный, минимум зелёного и синего)
		vec3(1.2f, 0.3f, 0.3f)     // specular - красновато-розовый
	);
