...
void initGraphicObjects()
{
    ...
    // Загружаем материалы
    int matDull = rm.loadMaterial("example/MATERIALS/dull_material.json");
    int matShiny = rm.loadMaterial("example/MATERIALS/shiny_material.json");
    ...
    graphicObject.setMaterialId(matDull);
    ...
    graphicObject.setMeshId(meshId);
    graphicObject.setTextureId(textureId);
    graphicObject.setMaterialId(matShiny);
    graphicObject.setColor(vec4(1.0f, 1.0f, 1.0f, 1.0f));
...
void display()
{
	...
	int FPS = getFps();
	snprintf(windowTitle, sizeof(windowTitle), "Laba_06 [%i FPS]", FPS);
...
int main(int argc, char** argv)
{
	...
	// Инициализируем второй источник света (вспомогательный, ФИОЛЕТОВЫЙ, сзади - "клубный" эффект)
	Light light2;
	light2.setDirection(glm::vec3(0.0f, -1.0f, -1.0f));  // Назад и вниз (клубный эффект на доме)
	light2.setAmbient(glm::vec4(0.05f, 0.05f, 0.05f, 1.0f));
	light2.setDiffuse(glm::vec4(0.6f, 0.0f, 0.8f, 1.0f));  // Яркий фиолетовый
	light2.setSpecular(glm::vec4(0.8f, 0.3f, 1.0f, 1.0f));  // Фиолетовый блик
	RenderManager::instance().setLight2(light2);
